-- Create trigger function to audit changes in dim_veiculo
-- This function captures all column changes and stores them in audit_dim_veiculo
--
-- Pipeline stage: Transform (Phase 3)
-- Populates core.audit_dim_veiculo on INSERT/UPDATE/DELETE on core.dim_veiculo.

CREATE OR REPLACE FUNCTION core.fn_audit_dim_veiculo()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        INSERT INTO core.audit_dim_veiculo (
            id_veiculo,
            operation,
            changed_fields,
            old_values,
...
COMMENT ON FUNCTION core.fn_audit_dim_veiculo() IS 'Trigger function to audit all changes in dim_veiculo table';

-- Create trigger
DROP TRIGGER IF EXISTS trg_audit_dim_veiculo ON core.dim_veiculo;

CREATE TRIGGER trg_audit_dim_veiculo
    AFTER INSERT OR UPDATE OR DELETE ON core.dim_veiculo
    FOR EACH ROW
    EXECUTE FUNCTION core.fn_audit_dim_veiculo();

COMMENT ON TRIGGER trg_audit_dim_veiculo ON core.dim_veiculo IS 'Trigger to automatically audit all changes in dim_veiculo';
